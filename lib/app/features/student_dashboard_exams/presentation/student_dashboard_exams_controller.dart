...
import 'package:qyam_novers_mobile/app/features/student_dashboard_exams/presentation/ui/student_dashboard_exams_ui_state.dart';
...
class StudentDashboardExamsController extends GetxController {
  //region State

  final state = StudentDashboardExamsUiState.defaultObj().obs;

  //endregion State

  //region Use Cases

  final StudentDashboardExamsUseCase _getExamsUseCase;
  ...
  StudentDashboardExamsController({
    required StudentDashboardExamsUseCase getExamsUseCase,
      ...
      () {
        if (state().scrollController.position.pixels ==
          ...
          if (state().pagedList.nextPage != -1 && !state().isLoading) {
            on(
              event: StudentDashboardExamsUiEvent.getExams(
  ...
  void on({
    required StudentDashboardExamsUiEvent event,
  }) {
  ...
  void _getExamsEvent({
    ...
    result.fold(
      ...
      (DataState<List<StudentExams>> dataState) async {
